...
class LegacyRNAModelClient(RNAModelClient):
    ...
    def restore(self, serializer, data, save=False, modified=True, **kwargs):
        for legacy, f in self.field_map.get(kwargs['model_class'], {}).items():
            data[f] = data.pop(legacy, None)
        return super(LegacyRNAModelClient, self).restore(
            serializer, data, save=save, modified=True, **kwargs)
